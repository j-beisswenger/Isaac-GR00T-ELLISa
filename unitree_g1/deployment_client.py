...
class UpperBodyController:
    ...
    def __init__(self, interface: str, kp: float = 10.0, kd: float = 1.5) -> None:
        self._crc = CRC()
        self._kp = kp
        self._kd = kd
    ...
    def send_targets(self, _left_arm: np.ndarray, _right_arm: np.ndarray) -> None:
        """Send 7-DoF arm targets and hold the waist at 0."""
        for j in range(16):
            left_arm = _left_arm[j]
            right_arm = _right_arm[j]
...
class CameraFeed:
    ...
    def next_frame(self) -> Optional[np.ndarray]:
        ...
        if frame_rgb.shape[0] != self._target_size_x or frame_rgb.shape[1] != self._target_size_y:
...
def build_observation(
    ...
    cv2.imwrite(f"tmp_frames/{uuid.uuid4()}.jpg", ego_view)
...
def parse_actions(raw_action: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray]:
    """Extract first-step arm targets; ignore hands and waist."""
    left = np.asarray(raw_action["action.left_arm"])
    right = np.asarray(raw_action["action.right_arm"])
    return left, right
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--kp", type=float, default=10.0, help="Position gain for upper body joints")
...
def main() -> int:
    ...
    try:
        while True:
            ...
            obs = build_observation(left_q, right_q, frame)
            action = policy.get_action(obs)
            left_target, right_target = parse_actions(action)
